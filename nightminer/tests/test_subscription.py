...
from nightminer.constants import LEVEL_DEBUG
...
def test_subscription():
    ...
    logging.debug('TEST: Scrypt algorithm = %r' % SCRYPT_LIBRARY, LEVEL_DEBUG)
    logging.debug('TEST: Testing Subscription', LEVEL_DEBUG)
    ...
    logging.debug('TEST: %r' % reply, LEVEL_DEBUG)
    ...
    for result in job.mine(nounce_start=1210450368 - 3):
        logging.debug('TEST: found share - %r' % repr(result), LEVEL_DEBUG)
        break

    valid = {'ntime': '52c7b81a', 'nounce': '482601c0', 'extranounce2': '00000000', 'job_id': u'1db7'}
    logging.debug('TEST: Correct answer %r' % valid, LEVEL_DEBUG)
